  ...
  return <>
    <div className="settings-grid">
      <SettingsSection title={t('settings.profile.title')} icon={settingsAssets.star}>
        <SettingsRow label={t('settings.profile.meName')} control={<input className="settings-text-input" aria-label={t('settings.profile.meName')} aria-invalid={!userNickname.trim()} maxLength={20} value={userNickname} onChange={(event) => setUserNickname(event.target.value)} onBlur={() => void persistNickname('user', userNickname)} />} />
        <SettingsRow label={t('settings.profile.partnerName')} control={<input className="settings-text-input" aria-label={t('settings.profile.partnerName')} aria-invalid={!partnerNickname.trim()} maxLength={20} value={partnerNickname} onChange={(event) => setPartnerNickname(event.target.value)} onBlur={() => void persistNickname('partner', partnerNickname)} />} />
