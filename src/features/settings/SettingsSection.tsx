...
export function SettingsRow({ icon, iconClassName = '', iconStyle, label, value, description, onClick, danger = false, unavailable = false, control, controlLayout = 'inline' }: { icon?: string; iconClassName?: string; iconStyle?: CSSProperties; label: string; value?: string; description?: string; onClick?: () => void; danger?: boolean; unavailable?: boolean; control?: ReactNode; controlLayout?: 'inline' | 'below' | 'responsive' }) {
  ...
  const usesStructuredControl = control && (controlLayout === 'below' || controlLayout === 'responsive')
  const content = usesStructuredControl
  ...
  const className = `settings-row ${icon ? '' : 'settings-row--no-icon'} ${danger ? 'settings-row--danger' : ''} ${unavailable ? 'settings-row--unavailable' : ''} ${controlLayout === 'below' && control ? 'settings-row--stacked-control' : ''} ${controlLayout === 'responsive' && control ? 'settings-row--responsive-control' : ''}`.trim()
