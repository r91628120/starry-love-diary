...
export function LikeOrHabitFlow({ onDone }: { onDone: () => void }) {
  ...
  return <section className="clear-flow">
    ...
    <SoftCard tone="green">
      ...
      {questions.map(([question, options]) => <fieldset key={question}><legend id={`like-or-habit-question-${question}`}>{t(key('clear.like.q.' + question))}</legend><div className="clear-answer-list" role="radiogroup" aria-labelledby={`like-or-habit-question-${question}`}>{options.map((option) => <button type="button" role="radio" key={option} className={answerFor(question) === option ? 'is-active' : ''} aria-checked={answerFor(question) === option} onClick={() => setSingle(section, question, option)}>{t(key('clear.like.answer.' + option))}</button>)}</div></fieldset>)}
