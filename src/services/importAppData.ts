...
function validateMoment(value: unknown): MemoryMoment { const record = object(value); return { ...datedBase(record), title: optionalText(record.title), content: text(record.content), order: number(record.order) } }
function validateMessageEntry(value: unknown): MessageToYouEntry { const record=object(value); return {...datedBase(record),type:enumValue(record.type,MESSAGE_TO_YOU_TYPES),content:text(record.content)} }
function validateRemembered(value: unknown): RememberedYouCard { const record = object(value); return { ...datedBase(record), title: text(record.title), content: text(record.content), isFavorite: boolean(record.isFavorite) } }
