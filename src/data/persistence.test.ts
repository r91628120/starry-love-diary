...
describe('Local persistence repositories', () => {
  ...
  it('validates, edits, deletes, keeps an unlimited archive and reopens heart phrases', async () => {
    ...
    const unicodePhrase = `${'字'.repeat(79)}💗`
    await expect(phrases.acceptHeartPhrase(unicodePhrase)).resolves.toMatchObject({ content: unicodePhrase })
    await expect(phrases.acceptHeartPhrase(`${unicodePhrase}😘`)).rejects.toThrow(/80/)
    ...
    expect((await reopened.getHeartPhrases()).map(({ content }) => content)).toEqual([unicodePhrase, '修改後'])
    await reopened.deleteHeartPhrase(created.id)
    expect((await reopened.getHeartPhrases()).map(({ content }) => content)).toEqual([unicodePhrase])
    await reopened.deleteHeartPhrase((await reopened.getHeartPhrases())[0].id)
